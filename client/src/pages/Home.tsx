...
export default function Home() {
  ...
  const renderAnnotatedImage = async () => {
    ...
    marks.forEach((mark) => {
      ...
      context.lineWidth = mark.tool === "arrow" ? (mark.arrowWidth ?? arrowWidth) * ((sx + sy) / 2) : mark.tool === "marker" ? 18 * ((sx + sy) / 2) : 2.2 * ((sx + sy) / 2);
      const markArrowStyle = mark.arrowStyle ?? arrowStyle;
      if (mark.tool === "arrow" && markArrowStyle !== "solid") context.setLineDash(markArrowStyle === "dashed" ? [10 * sx, 7 * sx] : [2 * sx, 7 * sx]);
